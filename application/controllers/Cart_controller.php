...
class Cart_controller extends Home_Core_Controller
{
    ...
    public function shipping()
    {
        ...
        if ($this->auth_check) {
            ...
            if (!empty($data["shipping_addresses"][0]->zip_code)) {
                $zip_code = $data["shipping_addresses"][0]->zip_code;
            }
            ...
            if (!empty($mds_cart_shipping)) {
                if (!empty($mds_cart_shipping->guest_shipping_address) && item_count($mds_cart_shipping->guest_shipping_address) > 0) {
                    if (!empty($mds_cart_shipping->guest_shipping_address['zip_code'])) {
                        //$state_id = $mds_cart_shipping->guest_shipping_address['state_id'];
                        $zip_code = $mds_cart_shipping->guest_shipping_address['zip_code'];
                    }
                }
            }
        }
        ...
        if (!empty($zip_code)) {
            $data["shipping_methods"] = $this->shipping_model->get_seller_shipping_methods_array($data['cart_items'], $zip_code);
            $data['show_shipping_cost'] = 1;
        }

        $data['selected_shipping_method_ids'] = [];
    ...
    public function get_shipping_methods_by_location()
    {
        ...
        foreach ($cart_items as $cartItem) {
            $product = $this->product_model->get_active_product($cartItem->product_id);
            if (empty($product->weight)) {
                $data = [
                    'result' => 1,
                    'html_content' => '<div class="alert alert-danger">All the products in a cart, must have a weight value.</div>',
                ];
                echo json_encode($data);
                exit;
            }
        ...
        // state_id is actually a zipcode here
        $zip_code = $this->input->post('state_id', true);
        
        
        if (!empty($zip_code)) {
            $vars = [
                "shipping_methods" => $this->shipping_model->get_seller_shipping_methods_array($cart_items, $zip_code)
            ];
    ...
    public function test_shipping()
    {
        ...
        $packages = [
            [
                ...
                'zip_origination' => '30253',
                'zip_destination' => '10002',
                'ounces' => '1.3',
                'container' => 'FLAT RATEBOX',
                'size' => 'REGULAR',
            ],
            [
        ...
        $rate_lookup = (array) $this->usps->shipping_rate_lookup($packages);
        
        echo '<pre>';
        print_r($rate_lookup['Package']);
    }
}

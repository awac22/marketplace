...
class Shipping_model extends CI_Model
{
    ...
    */
    ...
    public function get_seller_shipping_methods_array($cart_items, $state_id, $set_session = true)
    {
        ...
        if (!empty($cart_items)) {
            foreach ($cart_items as $item) {
                if ($item->product_type == "physical") {
                    $sellersCart[$item->seller_id][] = $item;
        ...
        foreach ($sellersCart as $sellerId => $cartItems) {
            $seller = get_user($sellerId);
            ...
            if (!empty($seller)) {
                ...
                foreach ($shipping_methods as $shipping_method) {
                    ...
                    //$method->code = $shipping_method['code'];
                    ...
                    $packages [] = [
                        'service' => $shipping_method['name'],
                        'zip_origination' => $seller->zip_code,
                        'zip_destination' => $state_id, // state id contains zip code
                        'ounces' => $weight, // update with item weight
                        'container' => $shipping_method['container'],
                        'size' => $shipping_method['size'],
                    ];
                    ...
                    $shippingCost = 0;
                    if (is_array($rate_lookup['Package'])) {
                        foreach ($rate_lookup['Package'] as $package) {
                            $shippingCost += $package->Postage->Rate;
                        }
                        $method->cost = $shippingCost;
                    } else {
                        $rate = (array)$rate_lookup['Package']->Postage->Rate;
                        $method->cost = $rate[0];
                    }
    ...
    public function add_shipping_zone_locations($zone_id)
    {
        ...
        if (!empty($country_ids)) {
            foreach ($country_ids as $country_id) {
                ...
                if (!empty($country)) {
                    //check if already exists
                    $zone_country = $this->db->where('country_id', clean_number($country_id))
                        ->where('zone_id', clean_number($zone_id))
